    ...
    <div className={`flex flex-col lg:flex-row gap-14 items-center ${isAr ? "rtl" : "ltr"}`}> 
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500">
                <span className="text-gray-800">{isAr ? "المقاس:" : "Size:"}</span> {item.size}
              </p>
              <p className="text-sm text-gray-500">
                <span className="text-gray-800">{isAr ? "اللون:" : "Color:"}</span> {item.color}
              </p>
      ...
      <div className="w-full lg:w-1/2 border border-gray-200 rounded-2xl p-4 transition-all duration-300 hover:shadow-md">
        ...
        <ul className="mb-4">
          ...
          <li className="flex justify-between items-center text-lg text-gray-500 mb-2 pb-2 border-b border-gray-100">
            <span>{isAr ? "خصم (-20%)" : "Discount (-20%)"}</span>
        ...
        <div className="flex items-center mb-5 gap-2">
          <input
            type="text"
            name="discount"
            placeholder={isAr ? "أدخل كود الخصم" : "Add promo code"}
            className="flex-1 bg-gray-200 placeholder:text-gray-400 p-2 rounded-full outline-none"
          />
          <button className="bg-pink-300 hover:bg-pink-400 text-white rounded-full px-4 py-2 h-12">
            {isAr ? "تطبيق" : "Apply"}
          </button>
        </div>

        <Link href="/Cart/checkout">
          <button className="w-full bg-pink-300 hover:bg-pink-400 text-white rounded-full h-16 text-lg font-medium flex items-center justify-center gap-3.5 transition-all">
